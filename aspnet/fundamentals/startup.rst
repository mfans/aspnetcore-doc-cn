...
ASP.NET Core 在应用程序启动期间提供了一些应用服务和对象。你可以非常简单地使用这些服务，只需要在在 ``Startup`` 类的构造函数或是它的 ``Configure`` 与 ``ConfigureServices`` 方法中一个包含合适的接口即可。下面定义了在 ``Startup`` 类中对每个方法可用的服务。框架服务和对象包括：
